...
class DataLoader():
    ...
    def load(self, load_part, cfg=None):
        global config
        if cfg is not None:
        ...
        if load_part == 'vali':
            return node_list_int[train_set:vali_set], time_list_int[train_set:vali_set], d2vec_list_int[
                                                                                         train_set:vali_set]
        if load_part == 'test':
    ...
    def get_triplets(self, cfg=None):
        if cfg is not None:
            config = yaml.safe_load(open(cfg))
        train_node_list, train_time_list, train_d2vec_list = self.load(load_part='train', cfg=cfg)
        ...
        for j in range(1, 1001):
            for i in anchor_index:
                ...
                if ok:
                    apn_id_triplets.append([i, p_index, n_index])  # idlist
        ...
        p = './data/{}/triplet/{}/'.format(config['dataset'], str(config["distance_type"]))
        ...
        pickle.dump(apn_id_triplets, open(str(config["path_id_triplets"]), 'wb'))

    def return_triplets_num(self):
        print(f"cfg.path_node_triplets:{config['path_node_triplets']}")
        apn_node_triplets = pickle.load(open(str(config["path_node_triplets"]), 'rb'))
        return len(apn_node_triplets)


def triplet_groud_truth(cfg=None):
    if cfg is not None:
        config = yaml.safe_load(open(cfg))
    apn_node_triplets = pickle.load(open(str(config["path_node_triplets"]), 'rb'))
...
def test_merge_st_dis(valiortest=None):
    s = np.load(
        './ground_truth/{}/{}/{}_spatial_distance.npy'.format(str(config["dataset"]), str(config["distance_type"]),
                                                              valiortest))
    t = np.load(
        './ground_truth/{}/{}/{}_temporal_distance.npy'.format(str(config["dataset"]), str(config["distance_type"]),
                                                               valiortest))
    print(s.shape)
...
class batch_list():
    def __init__(self, batch_size, nodes, d2vecs, cfg=None, ratio=1.0):
        if cfg is not None:
            config = yaml.safe_load(open(cfg))
        ...
        self.apn_node_triplets = pickle.load(open(str(config["path_node_triplets"]), 'rb'))
        self.apn_d2vec_triplets = pickle.load(open(str(config["path_d2vec_triplets"]), 'rb'))
        self.apn_id_triplets = pickle.load(open(str(config["path_id_triplets"]), 'rb'))
        for i in range(len(self.apn_node_triplets)):
            ids = self.apn_id_triplets[i]
            nodes_ = self.apn_node_triplets[i]
            d2vecs_ = self.apn_d2vec_triplets[i]
            assert nodes[ids[0]] == self.apn_node_triplets[i][0]
            assert nodes[ids[1]] == self.apn_node_triplets[i][1]
            assert nodes[ids[2]] == self.apn_node_triplets[i][2]
            assert d2vecs[ids[0]] == self.apn_d2vec_triplets[i][0]
            assert d2vecs[ids[1]] == self.apn_d2vec_triplets[i][1]
            assert d2vecs[ids[2]] == self.apn_d2vec_triplets[i][2]
        self.batch_size = batch_size
        self.ratio = ratio
        self.start = int(len(self.apn_node_triplets) * ratio)  # ordered is '0' ; reverse is 'maxsize'
    ...
    def getbatch_one(self):
        ...
        if self.start - self.batch_size < 0:
            self.start = int(len(self.apn_node_triplets) * self.ratio)
        batch_index = list(range(self.start - self.batch_size, self.start))
        self.start -= self.batch_size

        node_list, time_list, id_list = [], [], []
        # id_list = []
        for i in batch_index:
            node_list.append(self.apn_node_triplets[i])
            time_list.append(self.apn_d2vec_triplets[i])
            id_list.append(self.apn_id_triplets[i])
        ...
        n_id_batch = []
        for tri0 in id_list:
            a_id_batch.append(tri0[0])
            p_id_batch.append(tri0[1])
            n_id_batch.append(tri0[2])
        ...
        return a_node_batch, a_time_batch, p_node_batch, p_time_batch, n_node_batch, n_time_batch, batch_index, \
            (a_id_batch, p_id_batch, n_id_batch)
