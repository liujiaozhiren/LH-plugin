...
def batch_Point_distance():
    ...
    with Pool(cpu_num) as pool:
        for i in range(dataset_point + 1):
            if i != 0 and i % 1000 == 0:
                #parallel_point_com(i, list(range(i - 1000, i)))
                async_result = pool.apply_async(parallel_point_com, (i, list(range(i - 1000, i)), dataset_point, roadnetwork, dataset))
...
def merge_Point_distance():
    ...
    print(" store Point dis matrix")

def parallel_point_com(i, id_list = [], dataset_point=0, roadnetwork=None, dataset=""):
    batch_list = []
    try:
        for k in id_list:
            one_list = []
        ...
        print(f"An error occurred in task {i}: {e}")
        return f"An error occurred in task{i}:{e}"
...
def batch_similarity_ground_truth(valiortest = None, distance_matrix = None):
    node_list_int = np.load(str(config["shuffle_node_file"]), allow_pickle=True)
    if valiortest == 'vali':
    ...
    for i in range(len(sample_list)+1):
        if i!=0 and i % 50==0:
            rst = pool.apply_async(Traj_distance, (i, sample_list[i-50:i], node_list_int, valiortest, distance_matrix))
            async_results.append(rst)
    for async_result in async_results:
        async_result.wait()
        print("rst:", async_result.get())
    #pool.close()
    #pool.join()

    return len(sample_list)

def batch_similarity_ground_truth1(valiortest = None):
    node_list_int = np.load(str(config["shuffle_node_file"]), allow_pickle=True)
    if valiortest == 'vali':
        node_list_int = node_list_int[10000:14000]  # based dataset and "validation or test"  (train:validation:test = 1w:4k:1.6w)
    elif valiortest == 'test':
...
def merge_similarity_ground_truth(sample_len, valiortest):
    ...
    print(" store spatial dis matrix", './ground_truth/{}/{}/{}_spatial_distance.npy'.format(dataset, str(config["distance_type"]), valiortest))

def Traj_distance(k, sample_list = [[]], test_list = [[]], valiortest = None, distance_matrix = None):
    try:
        all_dis_list = []
        for sample in sample_list:
            ...
            for traj in test_list:
                if str(config["distance_type"]) == 'TP':
                    one_dis_list.append(TP_dis(sample, traj,distance_matrix))
                elif str(config["distance_type"]) == 'DITA':
                    one_dis_list.append(DITA_dis(sample, traj, distance_matrix))
                elif str(config["distance_type"]) == 'discret_frechet':
                    one_dis_list.append(frechet_dis(sample, traj, distance_matrix))
                elif str(config["distance_type"]) == 'LCRS':
                    one_dis_list.append(LCRS_dis(sample, traj, distance_matrix))
        ...
        return 'complete: ' + str(k)
    except Exception as e:
        # print(f"An error occurred in task {k}: {e}")
        return f"An error occurred in task{k}:{e}"
...
if __name__ == '__main__':
    ...
    node_edge_dict = generate_node_edge_interation()
    sample_len = batch_similarity_ground_truth(valiortest='vali',distance_matrix=distance_matrix)
    merge_similarity_ground_truth(sample_len=sample_len, valiortest='vali')
    sample_len = batch_similarity_ground_truth(valiortest='test',distance_matrix=distance_matrix)
