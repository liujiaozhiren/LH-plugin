...
class TrajGraphDataLoader:
    ...
    def get_data_loader(self):
        ...
        if self.phase == "val":
            data_loader = DataLoader(self.dataset, batch_size=batch_size, shuffle=is_shuffle, num_workers=0, collate_fn=self._collate_func)
        else:
    ...
    def _collate_func(self, samples):
        traj_list_list, dis_list_list, id_list = map(list, zip(*samples))
        trajdict_list_list = self._prepare(traj_list_list)
        trajgraph_list_list = self._build_graph(trajdict_list_list)

        return trajgraph_list_list, dis_list_list, id_list
