...
def train(city='chengdu', sim='dtw', model_type='lstm'):
    ...
    # lorentz = Lorentz(base_model=[], dim=(2, 128), lorentz=0, trajs=trajs)
    ...
    for epoch in range(500):
        ...
        if ratio > best_ratio:
            best_ratio = ratio
            early_stop = 0
            torch.save(model.state_dict(), f'./lorentz_mod_file/lorentz_{model_type}_{city}_{sim}_{best_ratio:.4f}_best.pth')
            print(f'Best ratio :{ratio:.4f} save ./lorentz_{city}_{sim}_best.pth')
        else:
            early_stop += 1
            if early_stop > 6:
                break
    ...
    # hr10, hr50, hr5, ndcg, hr10in50 = cal_top10_acc(dist, emb, (6000, 10000), lorentz, _10in50=True)
    #
