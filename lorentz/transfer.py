...
from lorentz.lorentz_net import Traj2Vector
# from neutraj.tools import config
import time

from lorentz.ndcg_msg import ndcg_

n = 4


class Lorentz(nn.Module):
    def __init__(self, dim, base_model, C=1, alpha=0.1, lorentz=0, trajs=None, load=None, model_type='lstm', sqrt=8.0,
                 net_init=None):
    ...
    def _cosh_encode(self, input) -> torch.Tensor:
        ...
        new_input2 = torch.zeros(target_shape, dtype=input.dtype, device=input.device)
        new_input[..., 0] = sq_c
        ...
        new_input2[...,1:] = new_input[..., 1:]
        new_input2[...,0] = torch.norm(new_input, dim=-1, p=2)
        # output[..., 1:] = new_input
    ...
    def _lorentz_distance(self, batch_x: torch.Tensor, batch_y: torch.Tensor) -> torch.Tensor:
    ...
    def _simple_lorentz_dist(self, batch_x: torch.Tensor, batch_y: torch.Tensor) -> torch.Tensor:
        lorentz_x = self._simple_encode(batch_x)
        lorentz_y = self._simple_encode(batch_y)
        # lorentz_dist = torch.abs(self.alpha.to(batch_x.device)) * (self._lorentz_distance(lorentz_x, lorentz_y) - self.C)
        lorentz_dist = self._lorentz_distance(lorentz_x, lorentz_y) - self.C
        return lorentz_dist

    def _cosh_lorentz_dist(self, batch_x: torch.Tensor, batch_y: torch.Tensor) -> torch.Tensor:

        # lorentz_x = self._cosh_encode(batch_x)
        # lorentz_y = self._cosh_encode(batch_y)
        lorentz_x, lorentz_y = self.__cosh(batch_x, batch_y)

        # lorentz_dist = torch.abs(self.alpha.to(batch_x.device)) * (self._lorentz_distance(lorentz_x, lorentz_y) - self.C)
        lorentz_dist = self._lorentz_distance(lorentz_x, lorentz_y) - self.C
        assert torch.exp(-lorentz_dist).isinf().any() == False
        return lorentz_dist

    def _cosh_lorentz_dist2(self, batch_x: torch.Tensor, batch_y: torch.Tensor) -> torch.Tensor:
        lorentz_x = self._cosh_encode2(batch_x)
        lorentz_y = self._cosh_encode2(batch_y)
        # lorentz_dist = torch.abs(self.alpha.to(batch_x.device)) * (self._lorentz_distance(lorentz_x, lorentz_y) - self.C)
        lorentz_dist = self._lorentz_distance(lorentz_x, lorentz_y) - self.C
        assert torch.exp(-lorentz_dist).isinf().any() == False
        return lorentz_dist
    ...
    def dist(self, batch_x: torch.Tensor, batch_y: torch.Tensor, config_lorentz=None) -> torch.Tensor:
        if self.lorentz == 0:
            _batch_x = batch_x.to(torch.float64)
            _batch_y = batch_y.to(torch.float64)
            return self._normal_dist(_batch_x, _batch_y)
        return self.cmb_dist(batch_x, batch_y, config_lorentz)



    def cmb_dist(self, batch_x: torch.Tensor, batch_y: torch.Tensor, config_lorentz=None) -> torch.Tensor:
        _batch_x = batch_x.to(torch.float64)
        _batch_y = batch_y.to(torch.float64)
        ...
        num_1or2 = int(self.lorentz * self.dim / 2)
        ...
        dist1 = self._simple_lorentz_dist(batch_x1, batch_y1)
        dist2 = self._cosh_lorentz_dist(batch_x2, batch_y2)
        if num_1or2 == 0:
            return dist0
        if num_0 == 0:
            return dist1 + dist2
    ...
    def learned_cmb_dist(self, batch_x: torch.Tensor, batch_y: torch.Tensor, traj_i=None, traj_j=None,
        ...
        dist0 = self._normal_dist(_batch_x, _batch_y).to(ori_device)
        # dist1 = self._simple_lorentz_dist(batch_x1, batch_y1)
        dist2 = self._cosh_lorentz_dist(batch_x2, batch_y2).to(ori_device)
        # dev = ori_device
        # return dist0 * ratio0.to(dev) + dist1 * ratio1.to(dev) + dist2 * ratio2.to(dev)
        return dist0 * ratio0.to(ori_device) + dist2 * ratio2.to(ori_device)
...
def cal_top10_acc(ground_truth, traj_embeddings, _range, lorentz: Lorentz, config_lorentz=None, quick=False, _10in50=False, extra_msg=False):
    with torch.no_grad():
        ...
        # Pre-generating lorentz ration
        lorentz.gen_valid_ratio_emb((_range[0], _range[-1]))

        for i in range(len(ground_truth)):
            traj_e = traj_embeddings[i].repeat(len(traj_embeddings), 1)
            # if lorentz.lorentz == 0 or (config_lorentz is not None and config_lorentz == 0) or config.lorentz == 0:
            if lorentz.lorentz == 0 or (config_lorentz is not None and config_lorentz == 0):
                fake_metrix[i, :] = torch.sum(torch.square(traj_e - traj_embeddings), dim=-1)
                # fake_metrix[i, :] = lorentz.dist(traj_e, traj_embeddings, config_lorentz)
            else:
                list_i, list_j = [i + _range[0] for _ in range(_range[0], _range[-1])], list(
                    range(_range[0], _range[-1]))
                # lorentz embedding
                fake_metrix[i, :] = lorentz.learned_cmb_dist(traj_e, traj_embeddings, list_i, list_j, train=False)
        ...
        if _10in50:
            ...
            if extra_msg:
                msg = ndcg_(ground_truth, fake_metrix)
                return acc_10, acc_50, acc5, ndcg_100, _10in50, (ground_truth, fake_metrix), msg
            return acc_10, acc_50, acc5, ndcg_100, _10in50, (ground_truth, fake_metrix)
        if extra_msg:
            msg = ndcg_(ground_truth, fake_metrix)
            return acc_10, acc_50, acc5, ndcg_100, (ground_truth, fake_metrix), msg
    return acc_10, acc_50, acc5, ndcg_100, (ground_truth, fake_metrix)


def cal_top10_acc_bak(ground_truth, traj_embeddings, _range, lorentz: Lorentz, config_lorentz=None, quick=False,
                      _10in50=False):
    with torch.no_grad():
        ground_truth = ground_truth[_range[0]:_range[-1] + 1, _range[0]:_range[-1] + 1]
        traj_embeddings = traj_embeddings[_range[0]:_range[-1] + 1]
...
def ndcg_at_k(y_true, y_pred, k=5):
    ...
    return np.nanmean(DCG / normalizer)
