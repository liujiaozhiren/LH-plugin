    ...
    def __init__(self, input_size, hidden_size, num_layers=3, trajs=None, model_type='lstm', device=None,
        ...
        if self.type == 'lstm':
            ...
            if load is not None and os.path.exists(
                    f'../lorenz/lorenz_{model_type}_{load[0]}_{load[1]}_0.0642_best.pth'):
                # f"lorenz_{model_type}_{city}_{sim}_{best_ratio:.4f}_best.pth"
                # self.load_state_dict(torch.load(f'../data_set/{load[0]}/model_{load[1]}_best.pth'))
                self.load_state_dict(torch.load(f'../lorenz/lorenz_{model_type}_{load[0]}_{load[1]}_0.0642_best.pth'))
                print('load pre train lorenz model:',
                      f'../lorenz/lorenz_{model_type}_{load[0]}_{load[1]}_0.0642_best.pth')
            else:
                print('not load pre train lorenz model:', f'../lorenz/lorenz_{model_type}_best.pth')
