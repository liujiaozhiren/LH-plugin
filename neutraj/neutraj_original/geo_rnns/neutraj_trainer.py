...
from lorentz.handler import LH_trainer
...
class NeuTrajTrainer(object):
    ...
    def data_prepare(self, griddatapath=config.gridxypath,
        ...
        if config.picked != 0:
            distance = distance[np.ix_(config.picked_idxs, config.picked_idxs)]
    ...
    def neutraj_train(self, print_batch=10, print_test=100, save_model=True, load_model=None,
        ...
        trainer_lh = LH_trainer(spatial_net, config.lorentz, every_epoch=3, grad_reduce=0.1)
        ...
        #
        #     spatial_net.lorentz.both_train(True, False)
        ...
        #
        #     if config.lorentz > 0:
        #         spatial_net.lorentz.both_train(False, True)
        #         if epoch % 3 == 0:
        ...
        #                     optimizer.zero_grad()
        #                     loss.backward()
        #                     for param in spatial_net.lorentz.traj2vec.parameters():
