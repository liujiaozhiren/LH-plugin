...
def trajectory_feature_generation(config,
    ...
    if config.picked != 0:
        picked_list = pickle.load(open('../picked_list.pkl', 'rb'))
        picked_idxs = picked_list[config.picked-1]
        # dist = dist[numpy.ix_(picked_idxs, picked_idxs)]
        new_trajs = []
        for idx in picked_idxs:
            new_trajs.append(trajs[idx])
        trajs = new_trajs
        config.picked_idxs = picked_idxs
        config.datalength = len(trajs)
    lons, lats = [], []
    ...
    if config.fname == 'chengdu':
        delta = 0.0002
    elif config.fname == 'porto':
        delta = 0.015
    elif config.fname == 'geolife':
        delta = 0.006
    elif config.fname.startswith("long."):
    ...
    preprocessor = Preprocesser(
        delta=delta, lat_range=lat_range, lon_range=lon_range)
