...
def test_comput_embeddings(self, spatial_net, test_batch=1025):
    if config.device == 'cuda':
        if config.recurrent_unit == 'GRU' or config.recurrent_unit == 'SimpleRNN':
            hidden = autograd.Variable(torch.zeros(
                test_batch, self.target_size), requires_grad=False).cuda()
        else:
        ...
        if config.recurrent_unit == 'GRU' or config.recurrent_unit == 'SimpleRNN':
            hidden = autograd.Variable(torch.zeros(
                test_batch, self.target_size), requires_grad=False).cpu()
        else:
            hidden = (autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).cpu(),
                      autograd.Variable(torch.zeros(test_batch, self.target_size), requires_grad=False).cpu())
        embeddings_list = []
        j = 0
        s = time.time()
        while j < self.padded_trajs.shape[0]:
...
def test_model(self, lorentz, traj_embeddings, test_range, print_batch=10, similarity=False, r10in50=False):
    ...
    for i in test_range:
        ...
        l_top10_recall = [l[0] for l in s_learned_distance[:11]
                          if l[0] in [j[0] for j in s_true_distance[:11]]]
        l_top50_recall = [l[0] for l in s_learned_distance[:51]
                          if l[0] in [j[0] for j in s_true_distance[:51]]]
